            ...
            <div class="col-sm-12" style="" id="account_transaction_list_div">
                ...
                @else
                    <table class="table table-striped table-bordered borderless" style="font-size: 14px">
                        ...
                        $start_balance = ProductsAndServicesController::getStartingAccrualBalance($date,$from,$account,"db");
                        if ($start_balance != 0)
                            $total = $start_balance;
                        ?>
                        ...
                        @foreach($items as $item)
                            <tr style="background-color: transparent;border-style: none;">
                                ...
                                @if(strpos($operation,"cr") !== false)
                                    <?php
                                    $total = $total - ($item->price*$item->quantity);
                                    $total_cr =  $total_cr + ($item->price*$item->quantity);
                                    ?>
                                    <td style="text-align: right"></td>

                                    <td style="text-align: right"><?php echo ProductsAndServicesController::money(($item->price*$item->quantity)); ?></td>
                                @elseif(strpos($operation,"db") !== false)
                                    <?php
                                    $total = $total + ($item->price*$item->quantity);
                                    $total_dr =  $total_dr + ($item->price*$item->quantity);
                                    ?>
                                    <td style="text-align: right"><?php echo ProductsAndServicesController::money(($item->price*$item->quantity)); ?></td>
                                    <td style="text-align: right"></td>
                                @endif
